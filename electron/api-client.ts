...
const apiBase = (
  process.env.REIS_API_URL ?? "http://localhost:3000/api/v1"
).replace(/\/+$/, "");
...
function normalizeAuth(value: unknown): AuthResponse {
  ...
  return {
    ...
    user: {
      ...
      phone: rawUser.phone
        ? String(rawUser.phone)
        : rawUser.telefone
          ? String(rawUser.telefone)
          : undefined,
      companyId: rawUser.companyId
        ? String(rawUser.companyId)
        : rawUser.empresaId
          ? String(rawUser.empresaId)
          : undefined,
      role:
        rawUser.role && typeof rawUser.role !== "object"
          ? String(rawUser.role)
          : rawUser.cargo &&
              typeof rawUser.cargo === "object" &&
              "nome" in rawUser.cargo
...
export async function publicSession(): Promise<PublicSession | null> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const updated = {
        ...session,
        user: { ...session.user, ...normalizePublicUser(rawUser) },
      };
...
export async function apiRequest(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `${message}${requestId ? ` (protocolo ${requestId})` : ""}`,
    );
  }
  return {
    data: unwrap(body),
...
export async function apiUpload(input: AllowedApiUpload): Promise<ApiResult> {
  const session = await loadSession();
