...
function unwrap<T>(body: unknown): T {
  if (
    ...
    if (!envelope.success) throw new Error(envelope.error?.message ?? envelope.message ?? 'Falha na API')
...
function normalizeAuth(body: unknown): StoredWebSession {
  ...
  if (!accessToken || !refreshToken || !data.user) throw new Error('Resposta de autenticação inválida')
  return {
    accessToken,
    refreshToken,
    expiresAt,
...
async function webFetch(path: string, init: RequestInit, retry = true): Promise<Response> {
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      throw new ReisApiError('A API demorou mais de 15 segundos para responder', 0)
    }
    throw new ReisApiError('Sem conexão com a API REIS', 0)
  } finally {
    window.clearTimeout(timeout)
  }
  if (response.status === 401 && retry && session && await refreshWebSession()) {
...
export const authApi = {
  async session(): Promise<PublicSession | null> {
    ...
    if (new Date(session.expiresAt).getTime() <= Date.now() && !await refreshWebSession()) {
      return null
    }
    const current = readWebSession()
  ...
  async login(input: LoginInput): Promise<PublicSession> {
    ...
    if (!response.ok) {
      ...
      throw new Error(body?.error?.message ?? body?.message ?? 'E-mail ou senha inválidos')
    }
  ...
  async logout() {
    ...
    const session = readWebSession()
    if (session) {
      await webFetch('/auth/logout', { method: 'POST' }, false).catch(() => undefined)
    }
    writeWebSession(null)
  },
}

export async function apiRequest<T>(input: AllowedApiRequest): Promise<ApiResult<T>> {
