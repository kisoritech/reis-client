...
function leadName(row: LeadRow) {
  return String(row.cliente?.nome ?? row.nome ?? row.name ?? 'Contato sem nome')
}
...
export default function LeadsPage({ search, refreshKey }: { search: string; refreshKey: number }) {
  ...
  const exportRows = () => {
    ...
    const csv = [
      ...
      ...rows.map((row) => [
        leadName(row), row.cliente?.email ?? row.email, row.cliente?.telefone ?? row.telefone ?? row.phone,
  ...
  return <section className="leads-page">
    ...
    {selectedLead?.id && <LeadDetails lead={selectedLead} statuses={statuses} onClose={() => setSelectedLead(null)} onChanged={() => { setSelectedLead(null); setVersion((value) => value + 1) }} />}
    <article className="panel leads-panel">
      <div className="lead-toolbar">
        <div className="lead-stages" role="tablist" aria-label="Etapas dos leads">{stageTabs.map((item) => <button type="button" role="tab" aria-selected={stage === item} className={stage === item ? 'active' : ''} key={item} onClick={() => setStage(item)}>{item}</button>)}</div>
      ...
      {!loading && !error && <div className="lead-table-scroll"><table className="lead-table"><thead><tr><th className="check-cell"><input type="checkbox" checked={allSelected} onChange={toggleAll} aria-label="Selecionar todos" /></th><th>Nome</th><th>Empresa / segmento</th><th>Status</th><th>Data de contato</th><th>Valor potencial</th><th>Score</th></tr></thead><tbody>
        {visible.map((row, index) => {
          ...
          const phone = row.cliente?.telefone ?? row.telefone ?? row.phone
          return <tr key={id} className={selected.has(id) ? 'selected' : ''} onClick={() => row.id && setSelectedLead(row)}><td className="check-cell"><input type="checkbox" checked={selected.has(id)} onClick={(event) => event.stopPropagation()} onChange={() => toggle(id)} aria-label={`Selecionar ${leadName(row)}`} /></td><td><div className="lead-person"><i>{initials(leadName(row))}</i><div><strong>{leadName(row)}</strong><small>{email ? String(email) : phone ? String(phone) : 'Sem contato informado'}</small></div></div></td><td>{String(row.origem ?? row.observacoes ?? row.segment ?? 'Não informado')}</td><td><span className={`lead-status status-${stageName(row).toLowerCase().replaceAll(' ', '-')}`}>{stageName(row)}</span></td><td>{formatDate(row.updatedAt ?? row.createdAt)}</td><td className="potential">{formatPotential(row)}</td><td><div className="score-cell"><span>{score ?? '—'}</span><i><b style={{ width: score === undefined ? '0%' : `${Math.max(0, Math.min(100, Number(score)))}%` }} /></i></div></td></tr>
        })}
...
function LeadDetails({ lead, statuses, onClose, onChanged }: { lead: LeadRow; statuses: LeadStatus[]; onClose: () => void; onChanged: () => void }) {
  const [history, setHistory] = useState<LeadHistory[]>([])
  const [statusId, setStatusId] = useState(
    typeof lead.status === 'object' && lead.status ? String((lead.status as { id?: string }).id ?? '') : '',
  )
  ...
  useEffect(() => {
    if (!lead.id) return
    apiRequest<LeadHistory[]>({ method: 'GET', path: `/crm/leads/${lead.id}/history` })
      .then((result) => setHistory(result.data))
      .catch((failure) => setError(failure instanceof Error ? failure.message : 'Falha ao carregar histórico'))
  }, [lead.id])
  const saveStatus = async () => {
    ...
    try {
      ...
      setError(failure instanceof Error ? failure.message : 'Não foi possível atualizar o lead')
      setWorking(false)
    }
  }
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="dialog lead-details"><div className="panel-heading"><div><span className="lead-status">{stageName(lead)}</span><h2>{leadName(lead)}</h2><p>{lead.cliente?.email ?? lead.cliente?.telefone ?? 'Contato não informado'}</p></div><button type="button" className="icon-button" onClick={onClose}><X /></button></div>
...
function NewLeadDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="dialog lead-dialog" onSubmit={submit}><div className="panel-heading"><div><h2>Novo lead</h2><p>Cadastre o cliente e o lead comercial na API REIS.</p></div><button type="button" className="icon-button" onClick={onClose}><X /></button></div><div className="dialog-fields"><label>Nome completo<input name="name" required autoFocus /></label><label>E-mail<input name="email" type="email" /></label><label>Telefone<input name="phone" /></label><label>CPF/CNPJ<input name="document" /></label><label>Empresa / segmento<input name="segment" /></label><label>Origem<input name="origin" placeholder="Indicação, WhatsApp, site…" /></label><label>Prioridade<select name="priority" defaultValue="media"><option value="baixa">Baixa</option><option value="media">Média</option><option value="alta">Alta</option></select></label><label>Score<input name="score" type="number" min="0" defaultValue="0" /></label><label>Valor potencial (R$)<input name="potentialValue" type="number" min="0" step=".01" defaultValue="0" /></label><label className="full-field">Observações<textarea name="notes" rows={3} /></label></div>{error && <div className="form-error">{error}</div>}<div className="dialog-actions"><button type="button" className="outline-button" onClick={onClose}>Cancelar</button><button className="gold-button" disabled={saving}>{saving ? 'Salvando…' : 'Salvar lead'}</button></div></form></div>
}
