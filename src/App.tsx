...
import CallRequestPrompt from "./CallRequestPrompt";
import "./App.css";
...
type AttendancePayload =
  DashboardAttendance[] | { items: DashboardAttendance[]; total?: number };
  ...
  return (
    <div className="brand">
      <div className="brand-mark" aria-hidden="true">
        <img src="/brand/reis-logo.png" alt="" />
      </div>
      <div>
...
function TopbarNotifications({
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await apiRequest<DayEvent[]>({
        method: "GET",
        path: `/calendar/events?${query}`,
      });
      setEvents(
        result.data
  ...
  const upcoming = events.filter(
    (event) => new Date(event.fim).getTime() >= now,
  );
  return (
    <div className="notification-center" ref={container}>
      <button
        type="button"
        className={`notification ${upcoming.length ? "has-events" : ""}`}
        aria-label={`${upcoming.length} agendamento(s) para hoje`}
        aria-expanded={open}
        onClick={() => setOpen((value) => !value)}
      >
        <Bell size={19} />
        {upcoming.length > 0 && (
          <i>{upcoming.length > 9 ? "9+" : upcoming.length}</i>
        )}
      </button>
      {open && (
                ...
                return (
                  ...
                  >
                    <span className="notification-time">
                      <Clock3 size={15} />
                      {new Date(event.inicio).toLocaleTimeString("pt-BR", {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </span>
...
export function LegacyDashboard({ refreshKey }: { refreshKey: number }) {
  const central = useRemote(
    async () =>
      (await apiRequest<Central>({ method: "GET", path: "/crm/central" })).data,
    [refreshKey],
  );
  ...
  const cards = [
    {
      ...
      detail: `${metrics.contacts} contatos`,
      icon: UsersRound,
    },
    {
      label: "Negócios ganhos",
      value: metrics.wonDeals,
      detail: `${metrics.openDeals} em aberto`,
      icon: CheckCircle2,
    },
    {
      label: "Tarefas concluídas",
      value: asNumber(tasks.concluidas),
      detail: `${metrics.pendingActivities} pendentes`,
      icon: Target,
    },
    {
      label: "Pipeline",
      value: money(metrics.pipelineValueCents),
      detail: `${asNumber(crm.taxaConversao)}% de conversão`,
  ...
  return (
    <>
      <div className="page-heading">
        <div>
          <h1>Dashboard</h1>
          <p>Visão geral atualizada diretamente pela API REIS.</p>
        </div>
        <button
      ...
      {analytics.error && (
        <div className="inline-warning">{analytics.error}</div>
      )}
      ...
      <section className="dashboard-grid">
        <article className="panel performance-panel">
          <div className="panel-heading">
            ...
            <span>{central.data.recentDeals.length} registros</span>
          </div>
          <DataTable rows={central.data.recentDeals} kind="deals" compact />
        </article>
        <article className="panel status-panel">
          <h2>Operação agora</h2>
          <ul className="summary-list">
            <li>
              <span>Negócios abertos</span>
            ...
            <li>
              ...
              <strong>{metrics.lostDeals}</strong>
            </li>
            <li>
              <span>Tarefas vencidas</span>
              <strong>{metrics.overdueActivities}</strong>
            </li>
          </ul>
        </article>
        <article className="panel sellers-panel wide-panel">
          <div className="panel-heading">
            <h2>Próximas atividades</h2>
            <span>Agenda operacional</span>
          </div>
          <DataTable
            rows={central.data.nextActivities}
            kind="activities"
            compact
          />
        </article>
...
function Dashboard({
  ...
  const normalizedStatus = (value?: string) =>
    (value ?? "aberto").toLocaleLowerCase("pt-BR");
  ...
  const completed = attendances.filter((item) =>
    finishedStatuses.includes(normalizedStatus(item.status)),
  );
  const lost = attendances.filter((item) =>
    lostStatuses.includes(normalizedStatus(item.status)),
  );
  const inProgress = Math.max(
    0,
    attendances.length - completed.length - lost.length,
  );
  const todaySchedules = schedules.filter(
    (item) =>
      new Date(item.inicio).toDateString() === now.toDateString() &&
      item.status !== "cancelado",
  );
  const conversion = attendances.length
    ? Math.round((completed.length / attendances.length) * 100)
    : 0;
  const cards = [
    {
      label: "Atendimentos",
    ...
    },
    {
      ...
      detail: `${todaySchedules.filter((item) => item.status === "confirmado").length} confirmados`,
      icon: CalendarCheck2,
    },
    {
      label: "Entregas realizadas",
      value: completed.length,
      detail: `${conversion}% dos atendimentos`,
      icon: CheckCircle2,
    },
    {
      label: "Resultado gerado",
      value: money(metrics.pipelineValueCents),
      detail: `${metrics.wonDeals} negócios ganhos`,
      icon: CircleDollarSign,
    },
  ];
  const anchors =
    period === "dia"
      ...
      : period === "mes"
        ...
        : Array.from(
            { length: 5 },
            (_, index) => new Date(now.getFullYear() - (4 - index), 0, 1),
          );
  const points = anchors.map((anchor) => {
    ...
    return {
      ...
      deliveries: completed.filter((item) =>
        matches(item.updatedAt ?? item.createdAt),
      ).length,
    };
  });
  const chartMax = Math.max(
    1,
    ...points.flatMap((item) => [item.attendances, item.deliveries]),
  );
  const upcoming = schedules
    .filter(
      (item) =>
        new Date(item.fim).getTime() >= Date.now() &&
        item.status !== "cancelado",
    )
    ...
    <>
      <div className="page-heading">
        <div>
          ...
          <p>
            Atendimentos, agenda, entregas e resultados comerciais em uma só
            visão.
          </p>
        </div>
        <button
      ...
      {operation.error && (
        <div className="inline-warning">
          Alguns dados operacionais não puderam ser atualizados:{" "}
          {operation.error}
        </div>
      )}
      <section className="metrics-grid">
      ...
      <section className="dashboard-grid results-dashboard">
        <article className="panel delivery-panel">
          <div className="panel-heading">
            <div>
              <h2>Atendimentos × entregas</h2>
              <span>Evolução dos resultados gerados</span>
            </div>
            <div className="chart-periods">
              {(["dia", "mes", "ano"] as const).map((item) => (
                <button
                  key={item}
          ...
          <div className="delivery-chart">
            {points.map((item) => (
              <div className="delivery-column" key={item.label}>
                <div className="delivery-bars">
                  <span
                    title={`${item.attendances} atendimentos`}
                    style={{
                      height: `${Math.max(item.attendances ? 8 : 0, (item.attendances / chartMax) * 100)}%`,
                    }}
                  >
                    <b>{item.attendances || ""}</b>
                  </span>
                  <span
                    title={`${item.deliveries} entregas`}
                    style={{
                      height: `${Math.max(item.deliveries ? 8 : 0, (item.deliveries / chartMax) * 100)}%`,
                    }}
                  >
                    <b>{item.deliveries || ""}</b>
                  </span>
        ...
        <article className="panel outcome-panel">
          <div className="panel-heading">
            <h2>Resultado dos atendimentos</h2>
            <button onClick={() => onNavigate("atendimentos")}>
              Ver todos
            </button>
          </div>
          <div
            className="outcome-ring"
            style={{ "--completed": `${conversion * 3.6}deg` } as CSSProperties}
          >
        ...
        <article className="panel agenda-panel">
          <div className="panel-heading">
            <div>
              ...
              <span>Compromissos ligados à operação</span>
            </div>
            <button onClick={() => onNavigate("agenda")}>Abrir agenda</button>
          </div>
          <div className="dashboard-agenda-list">
            {upcoming.length ? (
              upcoming.map((item) => (
                <button key={item.id} onClick={() => onNavigate("agenda")}>
                  <time>
                    <strong>
              ...
              <div className="empty">Nenhum agendamento futuro.</div>
            )}
          </div>
        </article>
        <article className="panel recent-attendance-panel">
          <div className="panel-heading">
            <div>
              <h2>Atendimentos recentes</h2>
              <span>Origem dos resultados comerciais</span>
            </div>
            <button onClick={() => onNavigate("atendimentos")}>
              Ver atendimentos
            </button>
              ...
              recent.map((item) => (
                ...
                >
                  <div>
                    ...
                    <span>
                      {item.tipoAtendimento?.nome ?? "Atendimento"} ·{" "}
                      {item.createdAt
                        ? new Date(item.createdAt).toLocaleDateString("pt-BR")
                        : "sem data"}
                    </span>
                  </div>
                  <span className="status-chip">{item.status ?? "aberto"}</span>
                </button>
              ))
            ) : (
      ...
      <table className="data-table">
        <thead>
          <tr>
            ...
            <th>Status</th>
            {!compact && (
              <th>{kind === "deals" ? "Valor" : "Contato / Prazo"}</th>
            )}
            {onComplete && <th />}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, index) => (
            <tr key={String(row.id ?? index)}>
              <td>
                <strong>{text(row, "nome", "name", "titulo", "title")}</strong>
                <small>{text(row, "email", "descricao", "description")}</small>
              </td>
              <td>
                <span className="status-chip">
                  {text(row, "status", "etapa", "stage")}
                </span>
              </td>
              {!compact && (
                <td>
                  {kind === "deals"
                    ? money(
                        asNumber(row.valueCents ?? row.valor) *
                          (row.valor ? 100 : 1),
                      )
                    : text(row, "telefone", "phone") !== "—"
                      ? text(row, "telefone", "phone")
                      : date(row.vencimento ?? row.dueAt)}
                </td>
              )}
              {onComplete && (
                <td>
                  <button
                    ...
                    onClick={() => onComplete(String(row.id))}
                  >
                    Concluir
                  </button>
                </td>
...
function ResourcePage({
  ...
  const remote = useRemote(
    async () =>
      (await apiRequest<Page<Row>>({ method: "GET", path: `${path}?${query}` }))
        .data,
    [path, page, search, refreshKey],
  );
  const complete = async (id: string) => {
    await apiRequest({
      method: "PATCH",
      path: `/crm/activities/${id}/complete`,
      body: {},
  ...
  return (
    <section className="module-page">
      <div className="page-heading">
        <div>
          <h1>{section.label}</h1>
          <p>{remote.data?.total ?? 0} registros encontrados na API.</p>
        </div>
        <button className="gold-button" onClick={() => setCreating(true)}>
          + Novo registro
        </button>
      </div>
      {creating && (
        ...
        {remote.data && (
          <>
            <DataTable
              ...
              onComplete={section.id === "fluxo" ? complete : undefined}
            />
            <div className="pagination">
              ...
              <span>
                Página {page} de {Math.max(remote.data.totalPages, 1)}
              </span>
              <button
                disabled={page >= remote.data.totalPages}
                onClick={() => setPage((v) => v + 1)}
              >
                Próxima
              </button>
...
function CreateDialog({
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    let path = "/crm/accounts";
    let body: Row = {
      name: values.name,
      email: values.email || undefined,
      phone: values.phone || undefined,
    };
    if (kind === "oportunidades") {
      path = "/crm/deals";
      body = {
        title: values.name,
        valueCents: Math.round(Number(values.value || 0) * 100),
        status: "OPEN",
      };
    }
    if (kind === "fluxo") {
...
export function LegacyReports({ refreshKey }: { refreshKey: number }) {
  ...
  const blocks = [
    ["CRM", remote.data.crm[0] ?? {}],
    ["Imobiliário", remote.data.imobiliario[0] ?? {}],
    ["Vendas", remote.data.vendas[0] ?? {}],
    ["Tarefas", remote.data.tarefas[0] ?? {}],
  ] as const;
  return (
    <>
      <div className="page-heading">
        <div>
          <h1>Relatórios</h1>
          <p>
            Indicadores consolidados em{" "}
            {new Date(remote.data.generatedAt).toLocaleString("pt-BR")}.
          </p>
        </div>
        <button className="period-button" onClick={remote.reload}>
          <RefreshCw size={15} /> Atualizar
        </button>
      </div>
...
export function AnalyticsReports({ refreshKey }: { refreshKey: number }) {
  ...
  const blocks = [
    ["CRM", report.crm[0] ?? {}],
    ["Imobiliário", report.imobiliario[0] ?? {}],
    ["Vendas", report.vendas[0] ?? {}],
    ["Tarefas", report.tarefas[0] ?? {}],
  ] as const;
  const cleanEntries = (values: Row) =>
    Object.entries(values).filter(([key]) => key !== "empresaId");
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      blocks.forEach(([title, values]) => {
        const rows = cleanEntries(values).map(([key, value]) => ({
          Indicador: label(key),
          Resultado: printable(value),
        }));
        const sheet = XLSX.utils.json_to_sheet(rows);
        sheet["!cols"] = [{ wch: 32 }, { wch: 24 }];
        XLSX.utils.book_append_sheet(workbook, sheet, title.slice(0, 31));
      });
      XLSX.writeFile(workbook, `relatorio-reis-${stamp}.xlsx`, {
        compression: true,
      });
    } catch (reason) {
      setExportError(
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      doc.setTextColor(218, 176, 37);
      doc.setFontSize(20);
      doc.text("REIS | Relatório de resultados", 16, 20);
      doc.setTextColor(210, 210, 210);
      doc.setFontSize(9);
      doc.text(`Gerado em ${new Date().toLocaleString("pt-BR")}`, 16, 29);
      y = 50;
      blocks.forEach(([title, values]) => {
        ensureSpace(20);
        ...
        cleanEntries(values).forEach(([key, value], index) => {
          ensureSpace(10);
          if (index % 2 === 0) {
            doc.setFillColor(246, 243, 234);
            doc.rect(14, y - 5.5, pageWidth - 28, 8, "F");
          }
          doc.setFontSize(9);
          doc.setFont("helvetica", "normal");
          doc.setTextColor(80, 80, 80);
          doc.text(label(key), 17, y);
          doc.setFont("helvetica", "bold");
          doc.setTextColor(25, 25, 25);
          doc.text(printable(value).slice(0, 55), pageWidth - 17, y, {
            align: "right",
          });
          y += 8;
        });
      ...
      for (let page = 1; page <= pages; page += 1) {
        ...
        doc.text(`Página ${page} de ${pages}`, pageWidth - 16, 291, {
          align: "right",
        });
      }
    ...
    <>
      <div className="page-heading report-heading">
        ...
        <div className="report-actions">
          <button
            className="outline-button excel-button"
      ...
      <section className="report-grid">
        {blocks.map(([title, values]) => (
          <article className="panel" key={title}>
            <h2>{title}</h2>
            <dl>
              {cleanEntries(values).map(([key, value]) => (
                <div key={key}>
                  <dt>{label(key)}</dt>
...
function Reports({ refreshKey }: { refreshKey: number }) {
  ...
  const relatedName = (item: DashboardAttendance, key: string) => {
    ...
    return value && typeof value === "object" && "nome" in value
      ? String((value as { nome?: unknown }).nome ?? "—")
      : "—";
  };
  ...
  const completed = attendances.filter((item) =>
    finishedStatuses.includes(normalize(item.status)),
  );
  const lost = attendances.filter((item) =>
    lostStatuses.includes(normalize(item.status)),
  );
  ...
  const statusCounts = Object.entries(
    attendances.reduce<Record<string, number>>((acc, item) => {
  ...
  const monthRows = Array.from({ length: 12 }, (_, month) => {
    const items = attendances.filter(
      (item) =>
        item.createdAt &&
    ...
    return {
      month: new Date(new Date().getFullYear(), month, 1).toLocaleDateString(
        "pt-BR",
        { month: "long" },
      ),
      count: items.length,
      ...
      value: items.reduce((sum, item) => sum + amount(item), 0),
    };
  });
  const rows = attendances
    ...
    .map((item) => ({
      ...
      Valor: amount(item),
      Observações: String(
        field(item, "observacoes", "descricao", "notes") ?? "",
      ),
    }));
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      const space = (height: number) => {
        if (y + height > 282) page();
      };
      doc.setFillColor(17, 17, 17);
      doc.rect(0, 0, width, 42, "F");
      doc.setTextColor(226, 188, 67);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(19);
      doc.text("REIS | Relatório de Atendimentos", 15, 19);
      doc.setFontSize(9);
      doc.setTextColor(210);
      doc.setFont("helvetica", "normal");
      doc.text(
        `Análise profissional gerada em ${new Date().toLocaleString("pt-BR")}`,
        15,
        29,
      );
      y = 53;
      ...
      const metrics = [
        ...
        ["Valor negociado", moneyValue(totalValue)],
        ["Agendamentos", schedules.length],
      ] as const;
      metrics.forEach(([name, value], index) => {
        const x = 15 + (index % 2) * 91;
        if (index > 0 && index % 2 === 0) y += 20;
        doc.setFillColor(247, 244, 235);
        doc.roundedRect(x, y, 84, 15, 2, 2, "F");
        doc.setFontSize(8);
        doc.setTextColor(90);
        doc.setFont("helvetica", "normal");
        doc.text(String(name), x + 4, y + 5);
        doc.setFontSize(11);
        doc.setTextColor(20);
        doc.setFont("helvetica", "bold");
      ...
      statusCounts.forEach(([status, count]) => {
        ...
        doc.setFont("helvetica", "bold");
        doc.text(
          `${count} (${attendances.length ? Math.round((count / attendances.length) * 100) : 0}%)`,
          191,
          y,
          { align: "right" },
        );
      ...
      doc.setFontSize(14);
      doc.setTextColor(20);
      doc.text("Detalhamento dos atendimentos", 15, y);
      y += 8;
      rows.forEach((row, index) => {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(25);
        doc.text(`${index + 1}. ${row.Cliente}`.slice(0, 80), 15, y + 3);
        doc.setFontSize(8);
        doc.setFont("helvetica", "normal");
        doc.setTextColor(80);
        doc.text(
          `${row.Data} | ${row.Tipo} | ${row.Status}`.slice(0, 105),
          15,
          y + 9,
        );
        doc.text(
          `Responsável: ${row.Responsável} | Empreendimento: ${row.Empreendimento}`.slice(
            0,
            105,
          ),
          15,
        ...
        doc.setFont("helvetica", "bold");
        doc.text(moneyValue(row.Valor), 195, y + 3, { align: "right" });
        y += 24;
      });
      const pages = doc.getNumberOfPages();
      for (let index = 1; index <= pages; index += 1) {
        ...
        doc.setFontSize(8);
        doc.setTextColor(130);
        doc.text(`Página ${index} de ${pages}`, 194, 291, { align: "right" });
      }
    ...
    <>
      <div className="page-heading report-heading">
        <div>
          ...
          <p>
            Análise detalhada da operação comercial, conversão e resultados
            gerados.
          </p>
        </div>
        <div className="report-actions">
          ...
          >
            <FileText size={17} />
            {exporting === "pdf" ? "Gerando…" : "PDF completo"}
          </button>
          <button className="period-button" onClick={remote.reload}>
            <RefreshCw size={15} /> Atualizar
          </button>
      ...
      <section className="attendance-report-metrics">
        <article>
          ...
          <small>{open} em andamento</small>
        </article>
        <article>
          <span>Taxa de conversão</span>
          <strong>{conversion}%</strong>
          <small>{completed.length} concluídos</small>
        </article>
        <article>
          <span>Valor negociado</span>
          <strong>{moneyValue(totalValue)}</strong>
          <small>Gerado pelos atendimentos</small>
        </article>
        <article>
          <span>Agendamentos</span>
          <strong>{schedules.length}</strong>
          <small>No ano corrente</small>
        </article>
      </section>
            ...
            {monthRows.map((item) => (
              <div key={item.month}>
                ...
                <div>
                  <i
                    style={{
                      width: `${attendances.length ? (item.count / Math.max(...monthRows.map((row) => row.count), 1)) * 100 : 0}%`,
                    }}
                  />
                </div>
                <strong>
          ...
          <div className="report-status-list">
            {statusCounts.map(([status, count]) => (
              <div key={status}>
                ...
                <small>
                  {attendances.length
                    ? Math.round((count / attendances.length) * 100)
                    : 0}
                  %
                </small>
              </div>
      ...
      <article className="panel attendance-report-table">
        <div className="panel-heading">
          <div>
            <h2>Detalhamento profissional</h2>
            <span>
              Cliente, atendimento, responsável, empreendimento e resultado
            </span>
          </div>
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr key={`${row.Cliente}-${index}`}>
                  ...
                  <td>
                    <strong>{moneyValue(row.Valor)}</strong>
                  </td>
                </tr>
              ))}
            </tbody>
    ...
    <main className="login-page">
      <form className="login-card" onSubmit={submit}>
        ...
        <div>
          ...
          <p>Use sua conta REIS para carregar os dados da empresa.</p>
        </div>
        <div
          className={`connection-status ${health.data ? "online" : health.error ? "offline" : ""}`}
        >
          <i />
          {health.data
            ? "API REIS conectada"
        ...
        <button
          className="gold-button"
          disabled={loading || Boolean(health.error)}
        >
...
function App() {
  ...
  useEffect(
    () =>
      window.reisDesktop?.deepLinks.subscribe((path) => {
  ...
  return (
    <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
        ...
        <div className="sidebar-bottom">
          ...
          >
            ...
            <div>
              ...
              <span>{session.user.role ?? "Usuário"}</span>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="workspace">
        <header className="topbar">
          ...
          <label className="search-box">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && activeId === "dashboard")
                  navigate("atendimentos");
              }}
              placeholder="Buscar no módulo atual…"
            />
            {query && (
        ...
        <main>
          {activeId === "dashboard" ? (
            <Dashboard refreshKey={refreshKey} onNavigate={navigate} />
          ) : activeId === "agenda" ? (
            ...
            <ClientsPage search={search} refreshKey={refreshKey} />
